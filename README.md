...
从 Releases 下载 `YouTube_downloader_1.4.1.exe` 后直接运行。程序已内置 FFmpeg 9.0、ffprobe 和 yt-dlp；当前构建支持 Windows 10 或更高版本。

年龄限制或需要登录的视频，请点击“登录 YouTube（年龄限制视频）”，在程序打开的独立浏览器窗口中完成 YouTube 登录和年龄验证。Chrome、Edge 或 Brave 登录成功后窗口会自动关闭；按钮显示“YouTube 已登录”后即可下载。程序会自动识别 Windows 默认浏览器；Cookie 只保存在本机的独立浏览器配置中，不会上传。
